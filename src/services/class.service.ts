...
export const listenToClasses = (
  ...
  const unsubscribe = onValue(classesRef, (snapshot) => {
    ...
    console.log("🔍 Raw classes keys:", Object.keys(val));
    callback(val);
  });
...
export const addStudent = async (
  ...
  const studentRef = ref(
    db,
    `professors/${professorId}/classes/${classId}/students/${studentData.studentId}`,
  );
...
export const updateStudent = async (
  ...
  classId: string,
  studentKey: string,
  updates: { name?: string; studentId?: string },
) => {
  const studentRef = ref(
    db,
    `professors/${professorId}/classes/${classId}/students/${studentKey}`,
  );
