...
export const MasterlistScreen = () => {
  ...
  const handleUpload = async () => {
    try {
      ...
      // Send directly to your Python server
      ...
      const response = await fetch(`${AI_SERVER_URL}/masterlist`, {
        method: "POST",
        body: formData,
      });
      const json = await response.json();
      if (!json.success) throw new Error(json.message || "Processing failed");

      for (const student of json.data) {
        await studentRepository.addStudent(uid, classId, {
          name: student.name,
          studentId:
            student.id ??
      ...
      showAlert(
        "Success",
        "Masterlist uploaded! The system is processing your roster — students will appear shortly.",
      );
    } catch (error: any) {
