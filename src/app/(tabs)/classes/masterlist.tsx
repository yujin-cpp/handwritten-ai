...
export default function Masterlist() {
  ...
  const handleUpload = async () => {
    try {
      ...
      const storagePath =
        `masterlists/${uid}/${classId}/` +
        `${fileAsset.name ?? "masterlist.pdf"}`;
      ...
      (data.data as any[]).forEach((student) => {
        const key = student.id || String(student.name).replace(/\s+/g, "_");
        students[key] = {
          name: student.name,
      ...
      await set(dbRef(db, `professors/${uid}/classes/${classId}/students`), students);

      showAlert(
        "Success",
        `Masterlist uploaded! ${data.data?.length ?? 0} students imported.`,
      );
    } catch (error: any) {
        ...
        {uploading ? (
          ...
          <View style={styles.statusSection}>
            <View style={styles.listHeaderRow}>
              ...
              <View
                style={[styles.activeBadge, { backgroundColor: headerColor + "15" }]}
              >
            ...
            <TouchableOpacity onPress={() => setModalVisible(true)}>
              <GlassCard color="rgba(255,255,255,0.5)" borderRadius={32}>
                <View style={styles.emptyCard}>
                  <View
                    style={[styles.largeIconBox, { backgroundColor: headerColor + "10" }]}
                  >
